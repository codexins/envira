...
if ( ! function_exists( 'codexin_posts_nav' ) ) {
    ...
    */
    function codexin_posts_nav( $prev = NULL, $next = NULL, $custom = NULL ) {
        $prev = ! empty( $prev ) ? $prev : esc_html__( 'Newer Posts', 'envira' );
        $next = ! empty( $next ) ? $next : esc_html__( 'Older Posts', 'envira' );
...
if ( ! function_exists( 'codexin_post_nav' ) ) {
    ...
    */
    function codexin_post_nav( $prev = NULL, $next = NULL ) {
        $prev = ! empty( $prev ) ? $prev : esc_html__( 'Previous Post', 'envira' );
        $next = ! empty( $next ) ? $next : esc_html__( 'Next Post', 'envira' );
        ...
        if( ( codexin_get_option( 'cx_post_pagination' ) == 'button' ) ) {
            ...
            $prev_link = get_previous_post_link( '%link', esc_html__( '%title &raquo;', 'envira' ) );
            $next_link = get_next_post_link( '%link', esc_html__( '&laquo; %title', 'envira' ) );
        }
    ...
    function codexin_comments_nav() {
        ...
        if ( get_comment_pages_count() > 1 && get_option( 'page_comments' ) ) {
            ...
            <nav id="comment-nav-below" class="navigation comment-navigation">
                <h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'envira' ); ?></h2>
                <div class="nav-links">
                    <div class="nav-previous"><?php previous_comments_link( esc_html__( '&laquo; Older Comments', 'envira' ) ); ?></div>
                    <div class="nav-next"><?php next_comments_link( esc_html__( 'Newer Comments &raquo;', 'envira' ) ); ?></div>
