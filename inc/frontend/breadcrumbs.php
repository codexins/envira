...
if( ! function_exists( 'codexin_breadcrumbs' ) ) {
    ...
    function codexin_breadcrumbs() {
        ...
        $home_link        = home_url('/');
        $home_text        = esc_html__( 'Home' , 'envira');
        ...
        if( is_page() ) {
            $bcrumb_position = ( ! empty( $bcrumb_single ) ) && ( $bcrumb_single != 'global' ) ? $bcrumb_single : '';
        }

        if( $bcrumb_position == 'flex-start' ) {
            $bcrumb_alignment_class = ' justify-content-md-start';
        } elseif( $bcrumb_position == 'flex-end' ) {
            $bcrumb_alignment_class = ' justify-content-md-end';
        } elseif( $bcrumb_position == 'center' ) {
            $bcrumb_alignment_class = ' justify-content-md-center';
        }
        ...
        if( is_archive() ) {
            if ( is_category() || is_tag() || is_tax() ) {
                ...
                $breadcrumb_trail = esc_html__( 'Articles Posted by&nbsp;', 'envira') .  $before . $queried_object->data->display_name . $after;
        ...
        if ( is_search() ) {
            $breadcrumb_trail = esc_html__( 'Search query for: ', 'envira' ) . $before . get_search_query() . $after;
        }

        // Handle 404's
        if ( is_404() ) {
            $breadcrumb_trail = $before . esc_html__( 'Error 404', 'envira' ) . $after;
        }

        // Handle paged pages
        if ( is_paged() ) {
            $current_page = get_query_var( 'paged' ) ? get_query_var( 'paged' ) : get_query_var( 'page' );
            $page_addon   = $before . sprintf( esc_html__( ' ( Page %s )', 'envira' ), number_format_i18n( $current_page ) ) . $after;
        }

        $breadcrumb_output_link  = '';
        $breadcrumb_output_link .= '<nav class="breadcrumb'. esc_attr( $bcrumb_alignment_class ) .'">';
