	...
	class Codexin_Sidebar_Widget {
		...
		public static function codexin_sidebar_widgets_init() {
			...
			register_sidebar( array(
				'name'				=> esc_html__( 'Sidebar (General)', 'envira' ),
				'id'				=> 'codexin-sidebar-general',
				'description'		=> esc_html__( 'This sidebar will show everywhere the sidebar is enabled, both Posts and Pages.', 'envira' ),
				'before_widget' 	=> '<div id="%1$s" class="%2$s sidebar-widget">',
				'after_widget'  	=> '</div>',			
			) );

			register_sidebar( array(
				'name'				=> esc_html__( 'Sidebar (Pages)', 'envira' ),
				'id'				=> 'codexin-sidebar-page',
				'description'		=> esc_html__( 'This sidebar will show on all Pages.', 'envira' ),
				'before_widget' 	=> '<div id="%1$s" class="%2$s sidebar-widget">',
				'after_widget'  	=> '</div>',		
			) );
			
			register_sidebar( array(
				'name' 				=> esc_html__( 'Sidebar (Blog)', 'envira' ),
				'id'				=> 'codexin-sidebar-blog',
				'description'		=> esc_html__( 'This sidebar will show on all blog Posts.', 'envira' ), 
				'before_widget' 	=> '<div id="%1$s" class="%2$s sidebar-widget">',
				'after_widget'  	=> '</div>',		
			) );
		} 
		...
		public static function codexin_footer_widgets() {
			...
			for( $i = 1; $i <= $widget_count ; $i++ ) { 
				register_sidebar( array(
					'name'				=> sprintf( esc_html__( 'Footer (Column-%s)', 'envira' ), $i ),
					'id'				=> 'codexin-footer-col-'. $i .'',
					'description'	 	=> sprintf( esc_html__( 'The widget area for the footer column %s', 'envira'), $i ),
